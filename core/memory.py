"""
Long-term memory manager for Бакыт.
...
from database.models import BakMemory, BakMessage, BakUser
...
async def get_or_create_user(
    ...
    user = await session.get(BakUser, user_id)
    if user is None:
        user = BakUser(
            id=user_id,
            username=username,
...
async def add_message(
    ...
    session.add(BakMessage(user_id=user_id, role=role, content=content))
    await session.commit()
...
async def get_history(
    ...
    result = await session.execute(
        select(BakMessage)
        .where(BakMessage.user_id == user_id)
        .order_by(BakMessage.created_at.desc())
        .limit(limit)
    )
...
async def clear_history(session: AsyncSession, user_id: int) -> int:
    result = await session.execute(
        delete(BakMessage).where(BakMessage.user_id == user_id)
    )
    await session.commit()
...
async def get_memories(
    ...
    result = await session.execute(
        select(BakMemory)
        .where(BakMemory.user_id == user_id)
        .order_by(BakMemory.updated_at.desc())
...
async def delete_memory(
    ...
    result = await session.execute(
        delete(BakMemory).where(BakMemory.user_id == user_id, BakMemory.key == key)
    )
    await session.commit()
...
- Расписание конкретного дня (выезд в 10 утра, ужин в 21:00)
- Что пользователь поел, приготовил, постирал
- Транзакции и покупки (они хранятся отдельно в финансах)
- Временные события (встреча, экзамен с конкретной датой)
...
async def extract_and_save_memories(
    ...
    personal_signals = (
        ...
        "факульт", "универ", "институт", "курс",
    )
    has_personal = any(sig in lower for sig in personal_signals)
    if not has_personal:
        return

    # Format message date for the LLM
    if message_date:
